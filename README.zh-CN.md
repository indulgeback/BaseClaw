...
Agents 页面内置工作、生活和专项等分类的数字员工；这些预置 Agent 会在首次加载时写入本地 OpenClaw 配置，因此打包后的桌面端也能直接使用。
用户也可以在 Agents 页面创建自己的数字员工，填写名称、一句话描述、角色策略和模型后，应用会把这些指令写入该 Agent 的本地 OpenClaw 工作区。
