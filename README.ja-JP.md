...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜclawxなのか">なぜSpriteClawなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**SpriteClaw**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、SpriteClawはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

SpriteClawはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。SpriteClawはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**

| 課題 | SpriteClawのソリューション |
|------|----------------------|
...
SpriteClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、SpriteClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
各 Agent は `provider/model` の実行時設定を個別に上書きできます。上書きしていない Agent は引き続きグローバルの既定モデルを継承します。

### 🗂️ 内蔵 Agents マーケット
Agents ページは、コミュニティの固定スナップショットから取り込んだ 203 件のテンプレートを表示するオフラインマーケットから始まります。内訳は The Agency / `msitarzewski/agency-agents`（`783f6a7`）、`mergisi/awesome-openclaw-agents`（`64055d5`）から厳選した Life カテゴリ、そしてオープンソースの旅行、占星術、タロット、MBTI、コンパニオン系 Agent から着想した Social Fun カテゴリです。テンプレートは 16 の上位カテゴリで表示され、アプリに同梱されるため実行時に GitHub へアクセスしません。追加時にのみ詳細を遅延読み込みし、OpenClaw ワークスペースファイル（`SOUL.md`、`AGENTS.md`、`IDENTITY.md`）へ変換します。
管理ビューでは既存の Agent 操作を維持しつつ、これらのワークスペースファイルを編集でき、任意の Agent のチャットへ直接移動できます。
...
SpriteClaw には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
互換ゲートウェイで `/models` が認証以外の理由で使えない場合、SpriteClaw は API キー検証時に軽量な `/chat/completions` または `/responses` プローブへ自動フォールバックします。

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。SpriteClawはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に SpriteClaw が自動的に起動します。
...
[Releases](https://github.com/indulgeback/BaseClaw/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
SpriteClawを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **言語と地域** – 使用する言語・地域の設定
2. **環境チェック** – 同梱ランタイムとゲートウェイの前提条件を確認
3. **必須スキル** – アプリ既定のスキルバンドルをインストール
4. **完了** – セットアップ完了後にメイン画面へ移動

サポート対象のシステム言語がある場合、ウィザードはその言語を初期選択し、未対応の場合は英語にフォールバックします。

AIプロバイダーやチャンネルの設定は、起動後に **設定** から行えます。

### プロキシ設定

SpriteClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、SpriteClawは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- SpriteClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
- SpriteClawのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
SpriteClawは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：

```┌─────────────────────────────────────────────────────────────────┐
│                        SpriteClaw デスクトップアプリ                    │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │              Electron メインプロセス                         │  │
...
- SpriteClaw は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit SpriteClaw** を使用してください。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がSpriteClawをより良くするのに役立ちます。
...
SpriteClawは優れたオープンソースプロジェクトの上に構築されています：
...
### SpriteClaw パートナープログラム 🚀

SpriteClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に SpriteClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、SpriteClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=indulgeback/BaseClaw&type=Date" alt="スター履歴チャート" />
</p>
...
SpriteClawは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
