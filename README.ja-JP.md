...
### 🗂️ 内蔵 Agents マーケット
Agents ページは、The Agency / `msitarzewski/agency-agents` の固定スナップショット（`783f6a7`）から取り込んだ 184 件のテンプレートを 14 の上位カテゴリで表示するオフラインマーケットから始まります。テンプレートはアプリに同梱され、実行時に GitHub へアクセスしません。追加時にのみ詳細を遅延読み込みし、OpenClaw ワークスペースファイル（`SOUL.md`、`AGENTS.md`、`IDENTITY.md`）へ変換します。
