...
Agents ページには業務、生活、専門分野の組み込みデジタル従業員が含まれます。これらのプリセット Agent は初回読み込み時にローカルの OpenClaw 設定へ登録されるため、配布されたデスクトップビルドでもそのまま利用できます。
ユーザーは Agents ページから独自のデジタル従業員も作成できます。名前、一文の説明、役割戦略、モデルを設定すると、その指示が対象 Agent のローカル OpenClaw ワークスペースへ書き込まれます。
