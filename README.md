...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="SpriteClaw Logo" />
</p>

<h1 align="center">SpriteClaw</h1>
...
<p align="center">
  ...
  <a href="#why-clawx">Why SpriteClaw</a> •
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/downloads/indulgeback/BaseClaw/total?color=%23027DEB" alt="Downloads" />
...
**SpriteClaw** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, SpriteClaw provides the interface you need to harness AI agents effectively.

SpriteClaw comes pre-configured with best-practice model providers and natively supports Windows as well as multi-language settings. Of course, you can also fine-tune advanced configurations via **Settings → Advanced → Developer Mode**.
...
Building AI agents shouldn't require mastering the command line. SpriteClaw was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | SpriteClaw Solution |
|-----------|----------------|
...
SpriteClaw is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
For custom channel account IDs, SpriteClaw enforces OpenClaw-compatible canonical IDs (`[a-z0-9_-]`, lowercase, max 64 chars, must start with a letter/number) to prevent routing mismatches.
SpriteClaw now also bundles Tencent's official personal WeChat channel plugin, so you can link WeChat directly from the Channels page with an in-app QR flow.
...
When a compatible gateway rejects `/models` for non-auth reasons, SpriteClaw automatically falls back to a lightweight `/chat/completions` or `/responses` probe during API key validation.

### 🌙 Adaptive Theming
Light mode, dark mode, or system-synchronized themes. SpriteClaw adapts to your preferences automatically.

### 🚀 Startup Launch Control
In **Settings → General**, you can enable **Launch at system startup** so SpriteClaw starts automatically after login.
...
Download the latest release for your platform from the [Releases](https://github.com/indulgeback/BaseClaw/releases) page.

#### Build from Source

```bash
# Clone the repository
git clone https://github.com/indulgeback/BaseClaw.git
cd SpriteClaw
...
SpriteClaw includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, SpriteClaw falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- SpriteClaw also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.
- Gateway restarts preserve an existing Telegram channel proxy if SpriteClaw proxy is currently disabled.
...
SpriteClaw employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:

```┌─────────────────────────────────────────────────────────────────┐
│                        SpriteClaw Desktop App                         │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
...
- SpriteClaw is an Electron app, so **one app instance normally appears as multiple OS processes** (main/renderer/zygote/utility). This is expected.
...
- Clicking the window close button (`X`) hides SpriteClaw to tray; it does **not** fully quit the app. Use tray menu **Quit SpriteClaw** for complete shutdown.
...
- uses a temporary SpriteClaw `userData` directory
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make SpriteClaw better.
...
SpriteClaw is built on the shoulders of excellent open-source projects:
...
### SpriteClaw Partner Program 🚀

We're launching the SpriteClaw Partner Program and looking for partners who can help introduce SpriteClaw to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the SpriteClaw team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=indulgeback/BaseClaw&type=Date" alt="Star History Chart" />
</p>
...
SpriteClaw is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
