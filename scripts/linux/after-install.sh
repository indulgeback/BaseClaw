...
# Post-installation script for SpriteClaw on Linux
...
# Create symbolic link for SpriteClaw app binary
if [ -x /opt/SpriteClaw/clawx ]; then
    ln -sf /opt/SpriteClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/SpriteClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/SpriteClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/SpriteClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/SpriteClaw/resources/apparmor-profile'
...
echo "SpriteClaw has been installed successfully."
